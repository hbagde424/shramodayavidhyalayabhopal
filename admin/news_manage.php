...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        if (in_array($ext, $allowed)) {
            $image_name = 'news_' . time() . '_' . rand(100, 999) . '.' . $ext;
    ...
    if ($edit_id > 0) {
        if ($image_name) {
            $old = mysqli_fetch_assoc(mysqli_query($conn, "SELECT image FROM news WHERE id=$edit_id"));
            if ($old && $old['image'] && file_exists($upload_dir . $old['image'])) unlink($upload_dir . $old['image']);
            $stmt = mysqli_prepare($conn, "UPDATE news SET title=?, description=?, news_date=?, is_active=?, image=? WHERE id=?");
            mysqli_stmt_bind_param($stmt, "sssisi", $title, $description, $news_date, $is_active, $image_name, $edit_id);
        } else {
            $stmt = mysqli_prepare($conn, "UPDATE news SET title=?, description=?, news_date=?, is_active=? WHERE id=?");
            mysqli_stmt_bind_param($stmt, "sssii", $title, $description, $news_date, $is_active, $edit_id);
        }
        mysqli_stmt_execute($stmt);
        header('Location: news_manage.php?msg=updated');
        exit;
    } else {
        $stmt = mysqli_prepare($conn, "INSERT INTO news (title, description, news_date, is_active, image) VALUES (?,?,?,?,?)");
        mysqli_stmt_bind_param($stmt, "sssis", $title, $description, $news_date, $is_active, $image_name);
        mysqli_stmt_execute($stmt);
...
if (isset($_GET['edit'])) {
    $id = (int) $_GET['edit'];
    $edit_row = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM news WHERE id=$id"));
}

$all_news = mysqli_query($conn, "SELECT * FROM news ORDER BY created_at DESC");
...
<div class="row g-4">
  <div class="col-md-4">
    <div class="info-card">
      <h6 class="mb-3"><?php echo $edit_row ? 'समाचार संपादित करें' : 'नया समाचार जोड़ें'; ?></h6>
      <form method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
          ...
          <input type="date" name="news_date" class="form-control" required value="<?php echo $edit_row ? $edit_row['news_date'] : date('Y-m-d'); ?>">
        </div>
        <div class="mb-3">
        ...
        <?php if ($edit_row): ?><a href="news_manage.php" class="btn btn-outline-secondary w-100 mt-2">रद्द करें</a><?php endif; ?>
      </form>
    </div>
  </div>
  ...
  <div class="col-md-8">
    <div class="info-card">
      <h6 class="mb-3">सभी समाचार</h6>
      <div class="table-responsive">
        <table class="table table-hover align-middle">
          <thead><tr><th>फोटो</th><th>शीर्षक</th><th>तारीख</th><th>स्थिति</th><th>क्रिया</th></tr></thead>
          <tbody>
          <?php while ($n = mysqli_fetch_assoc($all_news)): ?>
            <tr>
              <td><img src="<?php echo $n['image'] ? SITE_URL.'/assets/uploads/news/'.h($n['image']) : 'https://via.placeholder.com/60'; ?>" width="55" height="45" style="object-fit:cover;border-radius:6px;"></td>
              <td><?php echo h(mb_strimwidth($n['title'],0,40,'...')); ?></td>
              <td><?php echo date('d M Y', strtotime($n['news_date'])); ?></td>
